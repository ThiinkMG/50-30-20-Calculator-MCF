...
export function CategoryInsights() {
  ...
  // Function to get category items and totals
  const getCategoryItems = (category: "needs" | "wants" | "savings") => {
    if (!budget) return { items: [], total: 0 };
    
    const items = budget[category] || [];
    const total = items.reduce((sum, item) => sum + safeNumber(item.amount), 0);
    
    return { 
      items: items.map(item => ({
        name: item.name,
        amount: safeNumber(item.amount),
        percentage: total > 0 ? (safeNumber(item.amount) / total) * 100 : 0
      })),
      total
    };
  };

  const needsData = getCategoryItems("needs");
  const wantsData = getCategoryItems("wants");
  const savingsData = getCategoryItems("savings");

  // Function to sort items by amount (highest to lowest)
  const sortByAmount = (items: any[]) => {
              ...
              <div className="pt-2 pb-4">
                {needsData.items.length === 0 ? (
                  ...
                  <div className="space-y-4">
                    {sortByAmount(needsData.items).map((item, index) => (
            ...
            <AccordionContent>
              <div className="pt-2 pb-4">
                {wantsData.items.length === 0 ? (
                  ...
                  <div className="space-y-4">
                    {sortByAmount(wantsData.items).map((item, index) => (
                      <div key={index} className="space-y-1">
                        <div className="flex justify-between items-center text-sm">
                          <span className="font-medium">{item.name}</span>
                          <div className="flex items-center space-x-2">
            ...
            <AccordionContent>
              <div className="pt-2 pb-4">
                {savingsData.items.length === 0 ? (
                  ...
                  <div className="space-y-4">
                    {sortByAmount(savingsData.items).map((item, index) => (
                      <div key={index} className="space-y-1">
                        <div className="flex justify-between items-center text-sm">
                          <span className="font-medium">{item.name}</span>
                          <div className="flex items-center space-x-2">
