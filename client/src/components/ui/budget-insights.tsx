...
export function BudgetInsights() {
  ...
  const calculateInsights = (): BudgetInsight | null => {
    if (!isCalculated || !budget?.calculations) {
      console.log("Budget not calculated or missing calculations", budget);
      return null;
    }

    // Force console output to debug values
    console.log("Budget calculations:", budget.calculations);
    
    const totalIncome = safeNumber(budget.calculations.totalIncome);
    if (totalIncome === 0) {
      console.log("Total income is zero");
    ...
    console.log("Ideal amounts:", idealAmounts);

    // Use categoryTotals from calculations, falling back to empty values if not available
    let categoryTotals = budget.calculations.categoryTotals || { needs: 0, wants: 0, savings: 0 };
    console.log("Category totals:", categoryTotals);

    // Ensure we have numbers for all values
    const needsAmount = safeNumber(categoryTotals.needs);
    const wantsAmount = safeNumber(categoryTotals.wants);
    const savingsAmount = safeNumber(categoryTotals.savings);

    console.log("Amounts - Needs:", needsAmount, "Wants:", wantsAmount, "Savings:", savingsAmount);
    console.log("Total Income:", totalIncome);

    // Calculate percentages out of total income with extra safety checks
    // Will default to 0 if dividend or divisor is invalid
    const calculateSafePercentage = (amount: number, total: number): number => {
      if (total <= 0 || isNaN(amount) || isNaN(total)) return 0;
      return Math.round((amount / total) * 100);
    };
    
    const needsPercentage = calculateSafePercentage(needsAmount, totalIncome);
    const wantsPercentage = calculateSafePercentage(wantsAmount, totalIncome);
    const savingsPercentage = calculateSafePercentage(savingsAmount, totalIncome);
    
    console.log("Percentages - Needs:", needsPercentage, "Wants:", wantsPercentage, "Savings:", savingsPercentage);
    ...
    const THRESHOLD = 5; // percentage points difference to be considered on target

    const getStatus = (actual: number, target: number): 'on-target' | 'over' | 'under' => {
      // If the difference is within THRESHOLD percentage points, consider it on target
      if (Math.abs(actual - target) <= THRESHOLD) return 'on-target';
                  ...
                  <div className="relative pt-1">
                    <div className="h-2 bg-gray-200 dark:bg-gray-700 rounded">
                      <div
                        ...
                        style={{ width: `${Math.max(1, Math.min(100, insights.needs.percentage))}%` }}
                      ></div>
                    </div>
                  ...
                  <div className="relative pt-1">
                    <div className="h-2 bg-gray-200 dark:bg-gray-700 rounded">
                      <div
                        ...
                        style={{ width: `${Math.max(1, Math.min(100, insights.wants.percentage))}%` }}
                      ></div>
                    </div>
                  ...
                  <div className="relative pt-1">
                    <div className="h-2 bg-gray-200 dark:bg-gray-700 rounded">
                      <div
                        ...
                        style={{ width: `${Math.max(1, Math.min(100, insights.savings.percentage))}%` }}
                      ></div>
                    </div>
